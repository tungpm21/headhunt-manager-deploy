                ...
                return (
                  <tr key={emp.id} className="border-b border-border/50 hover:bg-background/50">
                    <td className="py-3 px-4">
                      <div>
                        <p className="font-medium text-foreground">{emp.companyName}</p>
          ...
          {Array.from({ length: data.totalPages }, (_, i) => i + 1).map((p) => (
            <Link
              ...
              className={`h-9 w-9 rounded-lg flex items-center justify-center text-sm font-medium transition-all ${
                p === page ? "bg-primary text-white" : "bg-surface border border-border text-muted hover:border-primary/30"
              }`}
            >
              {p}
            </Link>
