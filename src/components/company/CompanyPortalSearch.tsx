...
import { Briefcase, FileText, Loader2, Mail, Search, Send } from "lucide-react";
import {
  companyPortalSearch,
  type CompanyPortalSearchResultItem,
  type CompanyPortalSearchResultType,
} from "@/lib/company-portal-search";
...
export function CompanyPortalSearch() {
  const router = useRouter();
  const inputRef = useRef<HTMLInputElement>(null);
  const [isOpen, setIsOpen] = useState(false);
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<CompanyPortalSearchResultItem[]>([]);
  ...
  const debouncedSearch = useDebouncedCallback((nextQuery: string) => {
    startTransition(async () => {
      const nextResults = await companyPortalSearch(nextQuery);
  ...
  useEffect(() => {
    ...
    window.addEventListener("open-company-portal-search", handleOpen);

    return () => {
      document.removeEventListener("keydown", handleShortcut);
      window.removeEventListener("open-company-portal-search", handleOpen);
    };
  }, []);
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 p-4 pt-[12vh]">
      ...
      <div className="relative z-10 w-full max-w-2xl overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl">
        <div className="border-b border-border px-4 py-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Tìm tin tuyển dụng, ứng viên, job order, submission..."
        ...
        <div className="flex items-center justify-between border-t border-border bg-surface px-4 py-3 text-xs text-muted">
          ...
          <span>Mũi tên để di chuyển</span>
