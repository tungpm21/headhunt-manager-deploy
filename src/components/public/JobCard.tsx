...
export function JobCard({ job, compact = false }: JobCardProps) {
  return (
    ...
    >
      <article
        className={`relative flex h-full flex-col rounded-xl border border-[var(--color-fdi-border)] bg-[var(--color-fdi-paper)] shadow-[0_14px_34px_-30px_rgba(17,24,39,0.48)] transition-[background-color,border-color,box-shadow,transform] duration-200 ease-out group-hover:-translate-y-0.5 group-hover:border-[var(--color-fdi-border)] group-hover:bg-white group-hover:shadow-[0_22px_46px_-32px_rgba(17,24,39,0.58)] ${compact ? "min-h-[128px] p-3" : "min-h-[164px] p-4"}`}
      >
        {job.isFeatured && (
          <div
            className="absolute right-4 top-4 z-10 rounded-full bg-[var(--color-fdi-accent-orange)] px-2.5 py-1 text-xs font-bold uppercase text-white shadow-[0_10px_22px_-16px_rgba(242,92,36,0.9)]"
        ...
        <div className={`flex items-start pr-11 ${compact ? "gap-2.5" : "gap-3"}`}>
          <div
            className={`flex shrink-0 items-center justify-center overflow-hidden rounded-xl border border-[var(--color-fdi-border)] bg-white ${compact ? "h-10 w-10" : "h-12 w-12"}`}
            aria-hidden="true"
          >
            <LogoImage
        ...
        <div className={`flex flex-wrap items-center gap-x-3 gap-y-1.5 text-[var(--color-fdi-text-secondary)] ${compact ? "mt-1.5 text-xs" : "mt-2.5 text-sm"}`}>
          {job.location && (
          ...
          {job.workType && (
            <span className="rounded-full bg-[var(--color-fdi-surface)] px-2 py-1 text-xs font-semibold text-[var(--color-fdi-primary)]">
              {job.workType}
            </span>
          )}
        </div>
        ...
        {job.requiredLanguages.length > 0 && (
          <div className={`flex flex-wrap gap-1.5 ${compact ? "mt-2" : "mt-3"}`}>
            {job.requiredLanguages.map((lang) => (
              <span
                key={lang}
                className="inline-flex items-center gap-1 rounded-full bg-[var(--color-fdi-surface)] px-2 py-1 text-xs font-semibold text-[var(--color-fdi-primary)]"
              >
        ...
        <div className={`mt-auto flex items-center justify-between text-xs text-[var(--color-fdi-text-secondary)] ${compact ? "pt-2" : "pt-3"}`}>
