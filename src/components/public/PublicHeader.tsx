    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-100 shadow-sm">
      <div className="mx-auto max-w-7xl h-16 px-4 sm:px-6 lg:px-8 flex items-center gap-4">
          ...
          {navLinks.map((link) => (
            <div key={link.href} className="relative">
              ...
              >
                ...
                {link.label}
                {link.sub && (
                  <ChevronDown className={`h-3 w-3 transition-transform ${openDropdown === link.href ? "rotate-180" : ""}`} />
                )}
              </button>
        ...
        <div ref={search.containerRef} className="hidden lg:flex flex-1 max-w-md mx-auto relative">
          ...
          {showDropdown && (
            <div className="absolute top-full left-0 mt-1.5 bg-white rounded-xl border border-gray-200 shadow-2xl z-50 overflow-hidden"
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-5 max-h-[380px]">
                ...
                <div className="sm:col-span-2 sm:border-r border-gray-100 p-3 overflow-y-auto max-h-[380px]">
                  {hasEmployers && (
                    <div className="mb-2">
                      <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1 px-1">Có phải bạn đang tìm</p>
                      {suggestions!.employers.map((emp, i) => {
                        ...
                        return (
                          <button key={emp.id} type="button"
                            ...
                            <div className="h-6 w-6 rounded bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden shrink-0">
                            ...
                            <span className="text-[9px] text-gray-400 bg-gray-50 px-1 py-0.5 rounded shrink-0">Công ty</span>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  {hasKeywords && (
                    <div>
                      <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1 px-1">Từ khóa phổ biến</p>
                  ...
                  {hasJobs ? suggestions!.jobs.map((job, i) => {
                    ...
                    return (
                      <button key={job.id} type="button"
                        ...
                        <div className="h-8 w-8 rounded bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden shrink-0">
                          <LogoImage src={job.employer.logo} alt={job.employer.companyName} className="h-full w-full object-contain p-0.5" iconSize="h-3.5 w-3.5" />
                        </div>
        ...
        <button onClick={() => setMobileOpen(!mobileOpen)}
          className="md:hidden p-2 rounded-lg hover:bg-gray-100 transition-colors cursor-pointer ml-auto" aria-label="Toggle menu">
          {mobileOpen ? <X className="h-5 w-5 text-[var(--color-fdi-text)]" /> : <Menu className="h-5 w-5 text-[var(--color-fdi-text)]" />}
        </button>
      </div>
